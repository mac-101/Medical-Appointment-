...
const PatientLogin = ({ getUser, loggingIn, pickRole, clickCreate }) => {
  ...
  const handleSignUp = async (e) => {
    ...
    // 1. Force a role (Safety Check)
    const finalRole = selectedRole || 'patient';

    // 2. Streamlined Data Package
    const dataToSave = {
      fullName,
      email,
      password,
      role: finalRole,
      hospital: finalRole === 'doctor' ? hospital : null,
      specialty: finalRole === 'doctor' ? specialty : null,
      location: finalRole === 'doctor' ? location : null,
      ...
      <div className="flex items-center justify-center w-full bg-white p-8 sm:p-12 lg:p-20">
        <div className="w-full max-w-md">
          {!isLogin ? (
            <>
              {/* Plain Back Button */}
              <button onClick={pickRole} className="flex items-center text-slate-400 mb-8 hover:text-slate-900 transition-colors">
                <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 19l-7-7 7-7" /> 
                  
                </svg>
                <p className="text-sm mt-1">Select Role</p>
              </button>

              <div className="mb-10">
                <h1 className="text-4xl font-bold text-blue-600/80 font">{displayTitle}</h1>
              ...
              <form onSubmit={handleSignUp} className="space-y-5">
                ...
                {selectedRole === 'doctor' && (
                  <div className="animate-in fade-in slide-in-from-bottom-2">
                    <label className="block text-sm font-semibold text-slate-700 mb-1">Hospital of work</label>
                    <input
                      type="text"
                      placeholder="Central Hospital, NYC"
                      className="w-full px-4 py-3 rounded-lg border border-slate-200 focus:border-slate-900 outline-none transition-all"
                      value={hospital}
                ...
                <button
                  ...
                  className={`p-4 mt-4 rounded-lg font-bold text-lg transition-all flex justify-center items-center ${isLoading
                    ...
                    : "bg-blue-600/80 text-white hover:bg-blue-700/80 active:scale-[0.99]"
                    }`}
                >
