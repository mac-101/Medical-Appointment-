    ...
    <div className="max-w-5xl mx-auto p-2 space-y-10 pb-20">
      ...
      <div className="flex items-center justify-between border-b border-slate-100 pb-8">
        <div>
          ...
          <p className="text-slate-500 font-medium text-sm">Manage {isDoctor ? 'doctor' : 'user'} profile</p>
        </div>
        <button
          onClick={handleSave}
          disabled={loading}
        ...
        <div className="lg:col-span-4 flex flex-col items-center gap-6">
         ...
         <>
          <div className="relative group w-48 h-48">
            ...
            <label className="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 rounded-[3.5rem] transition-all cursor-pointer backdrop-blur-[2px] bg-black/20">
          ...
          <div className="text-center">
            ...
            <span className="text-[10px] font-black uppercase tracking-widest text-blue-600">{isDoctor ? 'Verified Doctor' : 'Patient'}</span>
          </div>
        </div>
            ...
            <div className="p-6 bg-slate-50 rounded-[2.5rem] space-y-6">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                ...
                <div className="flex gap-2 items-center">
                  <input type="time" className="bg-white px-3 py-1 rounded-lg font-bold border border-slate-100" value={formData.availabilityTime.start} onChange={(e) => setFormData({ ...formData, availabilityTime: { ...formData.availabilityTime, start: e.target.value } })} />
                  <span className="text-slate-300">to</span>
                  <input type="time" className="bg-white px-3 py-1 rounded-lg font-bold border border-slate-100" value={formData.availabilityTime.end} onChange={(e) => setFormData({ ...formData, availabilityTime: { ...formData.availabilityTime, end: e.target.value } })} />
                </div>
              </div>
              <div className="grid grid-cols-4 sm:grid-cols-7 gap-2">
                {DAYS.map(day => (
                  <button
                    ...
                    className={`py-3 rounded-xl text-[10px] font-black transition-all ${formData.availableDays.includes(day) ? 'bg-blue-600 text-white shadow-md' : 'bg-white text-slate-400 border border-slate-100 hover:border-blue-200'}`}
                  >
                    {day}
                  </button>
                ))}
